...
public class POOLController extends MainController {
	private POOLLayout mtLayout;
	private ArrayList<View >POOLList;
	private CustomAnimation mAnimation;
	private OSDDialogView mDialogView;
	private AnimationSet mAnimationSet;
	public POOLController(MainFragment frag) {
		super(frag);
	}

	protected void init() {
		mtLayout = (POOLLayout)frag.getView();
		mDialogView = new OSDDialogView(context);
		mAnimationSet = new AnimationSet(false);
		mAnimation = new CustomAnimation(context);
		POOLList = new ArrayList<View>();
		((FrameType)getFrameType()).getTitle().setText("叢集POOLS資訊");
		MONList();
		setOnItemListener();
	}
	...
	private void MONList() {
		for(int i=0 ; i<5 ; i++){
			...
			POOLList.add(mItemView);
		}
	
		mtLayout.getListView().setAdapter(new ListViewAdapter(POOLList));
	}
